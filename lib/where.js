...
function WhereSpec(where, spec) {
	if (where.field) { this.field = new SelectSpec(where.field, spec, where.table, where.data); }
	if (where.match) { this.match = new SelectSpec(where.match, spec, where.table, where.data); }
	if (where.or)    { this.or    = new Wheres(where.or, where.table, where.data); }
